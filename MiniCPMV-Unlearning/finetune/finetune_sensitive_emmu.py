...
from dataset import SupervisedDataset, SupervisedDatasetForUnlearning, DisturbanceDataset, EMMUDataset, LLMUDataset, emmu_data_collator, llmu_data_collator
from trainer_sensitive_emmu import CPMTrainer
...
def make_supervised_data_module(
    ...
    unlearning_json = json.load(open(data_args.unlearning_path, "r"))[50:]
    ...
    disturbance_json = json.load(open(data_args.disturbance_path, "r"))[50:]
    disturbance_dataset = DisturbanceDataset(disturbance_json)

    mismatch_json = json.load(open(data_args.mismatch_path, "r"))[50:]
    ...
    match_json = json.load(open(data_args.match_path, "r"))[50:]
    ...
    train_dataset = EMMUDataset(unlearning_dataset, disturbance_dataset, mismatch_dataset, match_dataset)
...
def train():
    ...
    data_module = make_supervised_data_module(
        ...
        data_collator=emmu_data_collator,
        slice_config=slice_config,
        llm_type=llm_type,
    ...
    if training_args.param_path:
        parameter_indices = json.load(open(training_args.param_path, "r"))
        params_pool = {}
        for idx, indices in enumerate(parameter_indices):
            layer = unlearned_model.llm.model.layers[target_layers[idx]].mlp
            param_pool = torch.zeros_like(layer.up_proj.weight)
            param_pool[indices, :] = 1
            params_pool[f'model.layers.{target_layers[idx]}.mlp.up_proj.weight'] = param_pool
