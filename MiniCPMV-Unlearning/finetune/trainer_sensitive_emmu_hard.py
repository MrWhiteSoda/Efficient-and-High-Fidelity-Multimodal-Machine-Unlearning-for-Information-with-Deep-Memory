...
class CPMTrainer(Trainer):
    def __init__(self, original_model=None, params_pool=None, param_scores=None, target_layers=None, slice_config=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        device = self.accelerator.device
        original_model = original_model.to(device).to(torch.bfloat16)
        self.original_model = original_model
        self.slice_config = slice_config
        self.params_pool = {k: v.to(device) for k,v in params_pool.items()}
        self.params_scores = param_scores.to(device)
    ...
    def _parameters_input_solver(self, unlearning_outputs, disturbance_outputs):
        ...
        min_per_layer = scores.min(dim=1, keepdim=True)[0]
        max_per_layer = scores.max(dim=1, keepdim=True)[0]
        normalized_scores = (scores - min_per_layer) / (max_per_layer - min_per_layer + 1e-8)
        del unlearning_outputs_tensor, disturbance_outputs_tensor
        torch.cuda.empty_cache()
    ...
    def _get_parameter_scores_only_input(self, model, data_batch, target_layers, tokenizer, device):
        model.eval()
        ...
        input_scores = self._parameters_input_solver(unlearning_first_token_outputs, disturbance_outputs)

        torch.cuda.empty_cache()

        return input_scores.to(device)
    ...
    # def training_step_only_update_batch_params(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        input_scores = self._get_parameter_scores_only_input(model, inputs['disturbance'], self.target_layers, self.tokenizer, self.args.device).unsqueeze(-1) # [L, d_out, 1]

        params_in_pool = {}
        for layer_idx in range(len(self.target_layers)):
            param_layer = self.params_scores[layer_idx].to(torch.float32)  # [d_out, d_in]
            input_layer = input_scores[layer_idx].to(torch.float32).expand_as(param_layer)  # [d_out, d_in]
            mask1 = input_layer >= torch.quantile(input_layer, 0.5)  # mask1: based on input
            mask2 = param_layer >= torch.quantile(param_layer, 0.5)  # mask2: based on param scores
            params = mask1 | mask2
            ...
            if weight_name in self.params_pool.keys():
                ...
                params_in_pool[weight_name] = params & param_pool
        torch.cuda.empty_cache()
        ...
        if self.params_pool:
            for name, param in self.model.llm.named_parameters():
                if name in params_in_pool.keys():
                    param.grad *= params_in_pool[name]
