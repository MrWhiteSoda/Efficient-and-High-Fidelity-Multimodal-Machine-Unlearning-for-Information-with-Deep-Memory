    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if isinstance(self.raw_data[i]["image"], str):
                ...
                images_dict = {img_name: Image.open(img_path).convert("RGB") for img_name, img_path in
                               self.raw_data[i]["image"].items()}
...
class SupervisedDatasetHard(Dataset):
    ...
    def __init__(
        ...
        super(SupervisedDatasetHard, self).__init__()
        self.raw_data = raw_data
        self.tokenizer = tokenizer
        self.transform = transform
        self.slice_config = slice_config
    ...
    def __getitem__(self, i) -> List[Dict[str, torch.Tensor]]:
        try:
...
class SupervisedDatasetForMulti(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(
        ...
        super(SupervisedDatasetForMulti, self).__init__()
        self.raw_data = raw_data
        self.tokenizer = tokenizer
        self.transform = transform
        self.slice_config = slice_config
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if isinstance(self.raw_data[i]["image"], str):
                ...
                images_dict = {"<image>": Image.open(self.raw_data[i]["image"]).convert("RGB").resize((1275, 1600))}
            elif isinstance(self.raw_data[i]["image"], Dict):
                images_dict = {img_name: Image.open(img_path).convert("RGB") for img_name, img_path in
                               self.raw_data[i]["image"].items()}
...
class SupervisedDatasetForUnlearning(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(
        ...
        super(SupervisedDatasetForUnlearning, self).__init__()
        self.raw_data = raw_data
        self.tokenizer = tokenizer
        self.transform = transform
        self.slice_config = slice_config
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            keywords_tokens = torch.tensor(self.raw_data[i]["keywords_tokens"], dtype=torch.int32)
            mask = torch.isin(ret["target"], keywords_tokens)
            masked_labels = torch.where(mask, ret["target"], torch.tensor(-100, dtype=torch.int32))
...
class SupervisedDatasetText(Dataset):
    """Dataset for supervised fine-tuning. (only text)"""
    ...
    def __init__(
        ...
        super(SupervisedDatasetText, self).__init__()
        self.raw_data = raw_data
        self.tokenizer = tokenizer
        self.llm_type = llm_type
        self.max_length = max_length
...
class DisturbanceDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            disturbance_image = Image.open(self.raw_data[i]["disturbance_image"]).convert("RGB")
...
class EMMUDataset(Dataset):
    def __init__(self, unlearning_dataset, disturbance_dataset, mismatch_dataset, match_dataset):
        self.unlearning_dataset = unlearning_dataset
        self.disturbance_dataset = disturbance_dataset
        self.mismatch_dataset = mismatch_dataset
        self.match_dataset = match_dataset

    def __len__(self):
        return len(self.unlearning_dataset)

    def __getitem__(self, idx):
        return {
            "unlearning": self.unlearning_dataset[idx],
            "disturbance": self.disturbance_dataset[idx],
            "mismatch": self.mismatch_dataset[idx],
            "match": self.match_dataset[idx],
        }


class EMMUDatasetHard(Dataset):
    # def __init__(self, unlearning_dataset, disturbance_dataset, mismatch_dataset, match_dataset, negative_dataset):
    def __init__(self, unlearning_dataset, disturbance_dataset, mismatch_dataset, match_dataset):
        self.unlearning_dataset = unlearning_dataset
        self.disturbance_dataset = disturbance_dataset
        self.mismatch_dataset = mismatch_dataset
...
class HardDataset(Dataset):
    def __init__(self, hard_dataset):
    ...
    def __getitem__(self, idx):
        return {
            "hard": self.hard_dataset[idx]
        }


def emmu_data_collator(features, max_length):
    if "unlearning" in features[0].keys():
        unlearning_features = [feature["unlearning"] for feature in features]
        disturbance_features = [feature["disturbance"] for feature in features]
        mismatch_features = [feature["mismatch"] for feature in features]
        match_features = [feature["match"] for feature in features]
        unlearning_batch = data_collator(unlearning_features, max_length=max_length)
        disturbance_batch = disturbance_collator(disturbance_features)
        mismatch_batch = data_collator(mismatch_features, max_length=max_length)
        match_batch = data_collator(match_features, max_length=max_length)

        batch = {
            "unlearning": unlearning_batch,
            "disturbance": disturbance_batch,
            "mismatch": mismatch_batch,
            "match": match_batch,
        }
    else:
        batch = data_collator(features, max_length=max_length)

    return batch


def emmu_data_collator_hard(features, max_length):
    if "unlearning" in features[0].keys():
        unlearning_features = [feature["unlearning"] for feature in features]
        disturbance_features = [feature["disturbance"] for feature in features]
        mismatch_features = [feature["mismatch"] for feature in features]
        mismatch_features = [item for feature in features for item in feature["mismatch"]]
        match_features = [feature["match"] for feature in features]
        unlearning_batch = data_collator(unlearning_features, max_length=max_length)
        disturbance_batch = disturbance_collator(disturbance_features)
        mismatch_batch = data_collator(mismatch_features, max_length=max_length)
...
def emmu_data_collator_hard1(features, max_length):
    if "unlearning" in features[0].keys():
        unlearning_features = [feature["unlearning"] for feature in features]
        disturbance_features = [feature["disturbance"] for feature in features]
        mismatch_features = [feature["mismatch"] for feature in features]
...
def data_collator_hard(features, max_length):
    if "hard" in features[0].keys():
        features_update = [item for feature in features for item in feature["hard"]]
        batch = data_collator(features_update, max_length=max_length)
    else:
        batch = data_collator(features, max_length=max_length)
    return batch
...
def data_collator_to_max_length(examples, padding_value=0, max_length=2048):

    def trim_and_pad(seq, batch_first, padding_value):
        padded_sequences = [s[:max_length] for s in seq]
        return pad_sequence(
            [torch.cat([s, torch.tensor([padding_value] * (1024 - len(s)))]) for s in padded_sequences],
...
def data_collator_text(examples, padding_value=0, max_length=2048):
...
class LLMUDataset(Dataset):
    def __init__(self, unlearning_dataset, mismatch_dataset, match_dataset):
        self.unlearning_dataset = unlearning_dataset
        self.mismatch_dataset = mismatch_dataset
        self.match_dataset = match_dataset
...
class MultiDeleteDataset(Dataset):
    def __init__(self, unlearning_dataset, unlearning_text_dataset, mismatch_dataset, match_dataset):
        self.unlearning_dataset = unlearning_dataset
        self.unlearning_dataset_text = unlearning_text_dataset
        self.mismatch_dataset = mismatch_dataset
        self.match_dataset = match_dataset
    ...
    def __getitem__(self, idx):
        return {
            "unlearning": self.unlearning_dataset[idx],
            "unlearning_text": self.unlearning_dataset_text[idx],
            "mismatch": self.mismatch_dataset[idx],
            "match": self.match_dataset[idx],
        }


def llmu_data_collator(features, max_length):
    if "unlearning" in features[0].keys():
        unlearning_features = [feature["unlearning"] for feature in features]
        mismatch_features = [feature["mismatch"] for feature in features]
        match_features = [feature["match"] for feature in features]
...
def multidelete_data_collator(features, max_length):
    if "unlearning" in features[0].keys():
        unlearning_features = [feature["unlearning"] for feature in features]
        unlearning_features_text = [feature["unlearning_text"] for feature in features]
        mismatch_features = [feature["mismatch"] for feature in features]
        match_features = [feature["match"] for feature in features]
        unlearning_batch = data_collator_to_max_length(unlearning_features, max_length=max_length)
        unlearning_text_batch = data_collator_text(unlearning_features_text, max_length=max_length)
        mismatch_batch = data_collator_to_max_length(mismatch_features, max_length=max_length)
        match_batch = data_collator(match_features, max_length=max_length)

        batch = {
            "unlearning": unlearning_batch,
            "unlearning_text": unlearning_text_batch,
            "mismatch": mismatch_batch,
            "match": match_batch,
        }
    else:
...
def conversation_to_ids(conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048):
    ...
    ids = torch.from_numpy(np.hstack([np.array(id, dtype=np.int32) for id in input_ids]))
    context = torch.from_numpy(np.hstack(context))
...
def conversation_to_ids_text(conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048):
...
def preprocess(
    ...
    for img_name, image in images_dict.items():
        if slice_config:
            source_image, patches, best_grid = slice_image(
                image,
                slice_config["max_slice_nums"],  # 9
                slice_config["scale_resolution"],  # 448
                slice_config["patch_size"],  # 14
            )
    ...
    if batch_vision:
        ...
        if tgt_sizes:
            tgt_sizes = torch.Tensor(tgt_sizes).type(torch.int32)  # 转换为 Tensor
...
def preprocess_text(
    ...
    if "<image>" in conversations[0]["content"]:
        conversations[0]["content"].replace("<image>", "")
    input_dict = conversation_to_ids_text(conversations, tokenizer, llm_type, max_length=max_length)
