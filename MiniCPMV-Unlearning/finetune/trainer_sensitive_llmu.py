...
class CPMTrainer(Trainer):
    def __init__(self, original_model=None, *args, **kwargs):
        ...
        # print("original_model_device:", self.original_model.device)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        prob_p = torch.nn.functional.softmax(original_model_outputs.logits, -1)
        prob_q = torch.nn.functional.softmax(unlearning_model_outputs.logits, -1)
        match_loss = -(prob_p * torch.log(prob_q + 1e-12)).sum(-1).mean()

        loss = unlearning_weight * unlearning_loss + mismatch_weight * mismatch_loss + match_weight * match_loss
