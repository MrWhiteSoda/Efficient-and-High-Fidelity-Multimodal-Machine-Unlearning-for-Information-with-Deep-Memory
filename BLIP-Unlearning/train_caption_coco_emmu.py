...
def train(original_model, unlearning_model, unlearning_loader, remain_loader, optimizer, epoch, params_pool, batch_params_indices, target_layers, device, config):
    ...
    for i, (unlearning_image, disturbance_image, unlearning_caption, _) in enumerate(
        ...
        parameter_indices = get_parameter_indices_only_input(unlearning_model.module, unlearning_image,
                                                             disturbance_image, target_layers, device)
        params_in_pool = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./unlearning_configs/emmu_configs/caption_coco_sensitive.yaml')
    parser.add_argument('--output_dir', default='./output/Caption_coco_emmu_resume')
