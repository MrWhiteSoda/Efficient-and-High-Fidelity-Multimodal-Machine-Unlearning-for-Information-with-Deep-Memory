...
def train(original_model, unlearning_model, unlearning_loader, remain_dismatch_loader, remain_match_loader, optimizer, epoch, device):
    ...
    for i, (unlearning_image, unlearning_caption, _) in enumerate(metric_logger.log_every(unlearning_loader, print_freq, header)):
        ...
        original_text = original_model.module.tokenizer(remain_dismatch_caption, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(remain_dismatch_image.device)
        ...
        unlearning_text = unlearning_model.module.tokenizer(unlearning_caption, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(unlearning_image.device)
        ...
        original_text_unlearning = original_model.module.tokenizer(unlearning_caption, padding='longest', truncation=True, max_length=35, return_tensors="pt").to(unlearning_image.device)
        ...
        unlearning_text_unlearning = unlearning_model.module.tokenizer(unlearning_caption, padding='longest', truncation=True, max_length=35, return_tensors="pt").to(unlearning_image.device)
        ...
        original_text_remain = original_model.module.tokenizer(remain_match_caption, padding='longest', truncation=True,
                                                 max_length=35, return_tensors="pt").to(remain_match_image.device)
...
def main(args, config):
    ...
    for epoch in range(0, config['max_epoch']):
        ...
        if utils.is_main_process():
            ...
            coco_val = coco_caption_eval(config['ann_root'], val_result_file, filename=config['val_gt'])
            coco_test = coco_caption_eval(config['ann_root'], test_result_file, filename=config['test_gt'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./unlearning_configs/mmu_configs/caption_coco.yaml')
    parser.add_argument('--output_dir', default='./output/Caption_coco_mmu_resume')
