...
def main(args, config):
    ...
    optimizer = torch.optim.AdamW(params=unlearning_model.parameters(), lr=config['init_lr'],
                                  weight_decay=config['weight_decay'])
    ...
    for epoch in range(0, config['max_epoch']):
        ...
        unlearning_result_file = save_result(unlearning_result, args.result_dir, 'unlearning_epoch%d' % epoch,
                                             remove_duplicate='image_id')
        ...
        if utils.is_main_process():
            ...
            if args.evaluate:
                log_stats = {**{f'val_{k}': v for k, v in coco_val.eval.items()},
                             ...
                             **{f'unlearning_{k}': v for k, v in coco_unlearning_val.eval.items()},
                             **{f'remain_{k}': v for k, v in coco_remain_val.eval.items()},
                             }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./unlearning_configs/train_configs/caption_flickr_salun.yaml')
    parser.add_argument('--output_dir', default='./output/Caption_flickr_salun_resume')
